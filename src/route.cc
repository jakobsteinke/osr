...
bidirectional_car_dijkstra& get_bidirectional_car_dijkstra() {
  static auto s = boost::thread_specific_ptr<bidirectional_car_dijkstra>{};
  if (s.get() == nullptr) {
    s.reset(new bidirectional_car_dijkstra{});
  }
  return *s.get();
}
...
routing_algorithm to_algorithm(std::string_view s) {
  switch (cista::hash(s)) {
    ...
    case cista::hash("bidirectional_car_dijkstra"): return routing_algorithm::kBidirectionalCarDijkstra;
  }
...
// Helper function to add path segments with shortcut unpacking
double add_path_with_shortcuts(ways const& w,
                               ...
                               direction const dir) {
  fmt::println("add_path_with_shortcuts: {} -> {} (expected_cost: {})", 
               from.get_node().v_, to.get_node().v_, expected_cost);
               
  // First, try to unpack any shortcuts between from and to
  auto const unpacked_path = bcd.unpack_path(from.get_node(), to.get_node());
  
  fmt::println("  Unpacked path has {} nodes", unpacked_path.size());
  for (auto i = 0U; i < unpacked_path.size(); ++i) {
    fmt::println("    [{}]: {}", i, unpacked_path[i].v_);
  }
  
  double total_dist = 0.0;
  
  if (unpacked_path.size() <= 2) {
    // Direct edge or no unpacking needed
    fmt::println("  Using direct add_path: {} -> {}", from.get_node().v_, to.get_node().v_);
    try {
      total_dist = add_path<car>(w, r, blocked, sharing, elevations, from, to, expected_cost, segments, dir);
    } catch (std::exception const& e) {
      fmt::println("  ERROR in direct add_path: {}", e.what());
      throw;
    }
  } else {
    // Shortcut was unpacked into multiple segments
    fmt::println("  Processing {} segments from unpacked path", unpacked_path.size() - 1);
    for (auto i = 0U; i + 1 < unpacked_path.size(); ++i) {
      auto const segment_from = unpacked_path[i];
      auto const segment_to = unpacked_path[i + 1];
      
      fmt::println("    Segment {}: {} -> {}", i, segment_from.v_, segment_to.v_);
      
      // Create car::node objects for each segment
      auto const from_node = car::node{segment_from, 0U, direction::kForward};
      auto const to_node = car::node{segment_to, 0U, direction::kForward};
      
      // Calculate proportional cost for this segment (simplified)
      auto const segment_cost = static_cast<cost_t>(expected_cost / (unpacked_path.size() - 1));
      
      try {
        auto const segment_dist = add_path<car>(w, r, blocked, sharing, elevations, from_node, to_node, 
                                                segment_cost, segments, dir);
        total_dist += segment_dist;
        fmt::println("      Segment {} OK: distance {}", i, segment_dist);
      } catch (std::exception const& e) {
        fmt::println("      ERROR in segment {}: {}", i, e.what());
        throw;
      }
    }
  }
  
  fmt::println("  add_path_with_shortcuts completed: total_dist {}", total_dist);
  return total_dist;
}
...
path reconstruct_bidirectional_car_dijkstra(ways const& w,
                                           ...
                                           cost_t const cost,
                                           direction const dir) {
  auto forward_n = bcd.meet_point_1_;
  ...
  while (true) {
    ...
    if (pred.has_value()) {
      ...
      forward_dist +=
          add_path_with_shortcuts(w, *w.r_, blocked, sharing, elevations, bcd, *pred,
                                  forward_n, expected_cost, forward_segments, dir);
    } else {
      break;
    }
  ...
  while (true) {
    ...
    if (pred.has_value()) {
      ...
      backward_dist += add_path_with_shortcuts(w, *w.r_, blocked, sharing, elevations, bcd,
                                            *pred, backward_n, expected_cost,
                                            backward_segments, opposite(dir));
    } else {
      break;
...
std::optional<path> route_bidirectional_car_dijkstra(ways const& w,
                                                     lookup const& l,
                                                     bidirectional_car_dijkstra& bcd,
                                                     location const& from,
                                                     location const& to,
                                                     match_view_t from_match,
                                                     match_view_t to_match,
  ...
  bcd.reset(max, from, to);

  for (auto const [i, start] : utl::enumerate(from_match)) {
    if (bcd.max_reached_1_ && component_seen(w, from_match, i)) {
      continue;
    }
    auto const start_way = start.way_;
    for (auto const* nc : {&start.left_, &start.right_}) {
    ...
    for (auto const [j, end] : utl::enumerate(to_match)) {
      ...
      if (bcd.max_reached_2_ && component_seen(w, to_match, j)) {
        continue;
      }
      auto const end_way = end.way_;
      for (auto const* nc : {&end.left_, &end.right_}) {
      ...
      auto const should_continue =
          bcd.run(w, *w.r_, max, blocked, sharing, elevations, dir);

      if (bcd.meet_point_1_.get_node() == node_idx_t::invalid()) {
        if (should_continue) {
          continue;
        }
        return std::nullopt;
      ...
      auto const cost = bcd.get_cost_to_mp(bcd.meet_point_1_, bcd.meet_point_2_);

      return reconstruct_bidirectional_car_dijkstra(w, l, blocked, sharing, elevations, bcd, from, to,
                          start, end, cost, dir);
    }
    bcd.pq1_.clear();
    bcd.pq2_.clear();
...
std::optional<path> route_bidirectional_car_dijkstra(ways const& w,
  ...
  if (profile != search_profile::kCar) {
    throw utl::fail("bidirectional car dijkstra only supports car profile");
  }
  
  auto const from_match =
      l.match<car>(from, false, dir, max_match_distance, blocked);
  auto const to_match =
      l.match<car>(to, true, dir, max_match_distance, blocked);

  if (from_match.empty() || to_match.empty()) {
    return std::nullopt;
  }

  return route_bidirectional_car_dijkstra(w, l, get_bidirectional_car_dijkstra(),
...
std::vector<std::optional<path>> route(
  ...
  return with_profile(profile, [&]<typename Profile>(Profile&&) {
    return route(w, l, get_dijkstra<Profile>(), from, to, from_match, to_match,
                 max, dir, blocked, sharing, elevations, do_reconstruct);
  });
}
...
std::optional<path> route(ways const& w,
  ...
  switch (algo) {
    ...
    case routing_algorithm::kBidirectionalCarDijkstra:
      if (profile != search_profile::kCar) {
        throw utl::fail("bidirectional car dijkstra only supports car profile");
      }
      return route_bidirectional_car_dijkstra(w, l, get_bidirectional_car_dijkstra(),
  ...
  switch (algo) {
    ...
    case routing_algorithm::kAStarBi:
      return route_bidirectional(w, l, profile, from, to, max, dir,
                                 ...
                                 elevations);
    case routing_algorithm::kBidirectionalCarDijkstra:
      return route_bidirectional_car_dijkstra(w, l, profile, from, to, max, dir,
                                              max_match_distance, blocked, sharing,
                                              elevations);
  }
  throw utl::fail("not implemented");

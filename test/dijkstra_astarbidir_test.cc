...
void run(ways const& w,
  ...
  auto const single_run = [&](std::pair<node_idx_t, node_idx_t> const from_to) {
    ...
    auto const ch_result =
        route(w, l, search_profile::kCar, from_loc, to_loc, from_matches_span,
              to_matches_span, max_cost, direction::kForward, nullptr, nullptr,
              nullptr, routing_algorithm::kCHDijkstra);
    auto const ch_time =
        std::chrono::steady_clock::now() - ch_start;

    bool ch_matches = true;
    if (reference.has_value() != ch_result.has_value() ||
        (reference && ch_result && reference->cost_ != ch_result->cost_)) {
      ch_matches = false;
    }
    ...
    if (reference.has_value() != experiment.has_value() ||
      ...
      auto const print_result = [&](std::string_view name, auto const& p,
        ...
        if (p.has_value() && kPrintDebugGeojson) {
          ...
          fmt::println("Debug geojson printing not implemented\n");
        }
      };
      ...
      if (!ch_matches) {
        print_result("ch dijkstra", ch_result, ch_time);
      }
        ...
        static_cast<double>(
            ...
            static_cast<double>(
                std::reduce(begin(experiment_times), end(experiment_times))
                    .count()));
    fmt::println(
        "CH speedup on non-empty: {:.2f}",
        static_cast<double>(
            std::reduce(begin(reference_times), end(reference_times)).count()) /
            static_cast<double>(
                std::reduce(begin(ch_times), end(ch_times)).count()));
  }
}
